...
import com.thomsonreuters.scholarone.archivefiles.StatInfo;
...
public class S3FileUtil
{
  ...
  private static String CACHE_PATH = ConfigPropertyValues.getProperty("archive.cache.dir");
  ...
  public static File getFile(String key, String bucketName) throws S3FileNotFoundException
  {
    ...
    {
      log.error("[ " + ServiceComponent.ARCHIVE_S1SVC.getComponent() +" | ID-5001 | GET-FILE-ERROR | S3FileUtil | (" + bucketName + ":" + key + ") | "
  ...
  public static void putFile(String key, File file, String bucketName) throws S3FileNotFoundException
  {
    ...
    {      
      ...
      log.info("[ " + ServiceComponent.ARCHIVE_S1SVC.getComponent() + " | ID-5002 | PUT-FILE-SUCCESS | S3FileUtil | " + "(" + bucketName + ":" + key + ")"
          + " | " + (end - start) + " ms ]");

    }
    catch(AmazonServiceException ase)
    {
      log.error("[ " + ServiceComponent.ARCHIVE_S1SVC.getComponent() + " | ID-5002 | PUT-FILE-ERROR | S3FileUtil | "
  ...
  public static int removeFile(String key, String bucketName)
  {
    ...
    try
    {
      ...
      log.info("[ " + ServiceComponent.ARCHIVE_S1SVC.getComponent() +" | ID-5003 | DELETE-FILE-SUCCESS | S3FileUtil | " + "(" + bucketName + ":" + key + ")" + " ]");
    }
    catch (AmazonServiceException amazonServiceException)
    {
      log.error("[ " + ServiceComponent.ARCHIVE_S1SVC.getComponent() +" | ID-5003 | DELETE-FILE-ERROR | S3FileUtil | " + "(" + bucketName + ":" + key + ")" + " | "
  ...
  public static int copyS3Dir(S3File sourceS3Dir, S3File destinationS3Dir, ArrayList<String> exclusionList, StorageClass storageClass, StatInfo stat)
  {
    ...
    if (fileKeys != null)
    {
      ...
      {
        PublishMetrics.incrementTotalCount  (MetricProduct.S1M, ServiceComponent.ARCHIVE_S1SVC.getComponent(), IMetricSubTypeConstants.ARCHIVE_FILES, envType, envName);
        PublishMetrics.incrementSuccessCount(MetricProduct.S1M, ServiceComponent.ARCHIVE_S1SVC.getComponent(), IMetricSubTypeConstants.ARCHIVE_FILES, envType, envName);
      }
      for (int i = 0; i < fileKeys.length; i++)
      {
        ...
        try 
        {
          ...
          PublishMetrics.logExecutionTime(MetricProduct.S1M, (int)(end - start), ServiceComponent.ARCHIVE_S1SVC.getComponent(), IMetricSubTypeConstants.ARCHIVE_FILES, envType, envName);
          if (cor == null | cor.getETag() == null)
          {
            PublishMetrics.incrementFailureCount(MetricProduct.S1M, ServiceComponent.ARCHIVE_S1SVC.getComponent(), IMetricSubTypeConstants.ARCHIVE_FILES, envType, envName);
          }
          else
          {          
            ...
            else
            {
              ...
              PublishMetrics.incrementSuccessCount(MetricProduct.S1M, ServiceComponent.ARCHIVE_S1SVC.getComponent(), IMetricSubTypeConstants.ARCHIVE_FILES, envType, envName);
              log.info("[ " + ServiceComponent.ARCHIVE_S1SVC.getComponent() +" | ID-5004 | COPY-DIR-SUCCESS | S3FileUtil | " + srcFullPath + " | " + destFullPath + " ]");
            }
          }
        }
        catch (AmazonServiceException ase)
        {
            ...
            PublishMetrics.incrementErrorCount(MetricProduct.S1M, ServiceComponent.ARCHIVE_S1SVC.getComponent(), IMetricSubTypeConstants.ARCHIVE_FILES, envType, envName);
            log.error("[ " + ServiceComponent.ARCHIVE_S1SVC.getComponent() +" | ID-5004 | COPY-DIR-ERROR | S3FileUtil | " + srcFullPath + " | " + destFullPath + " | "  
                + ase.getMessage() + " ]");
        }
        catch(SdkClientException sce)
        {
    ...
    else
    {
      stat.increaseNumOfFilesFailure();
      PublishMetrics.incrementFailureCount(MetricProduct.S1M, ServiceComponent.ARCHIVE_S1SVC.getComponent(), IMetricSubTypeConstants.ARCHIVE_FILES, envType, envName);
      log.error("[ " + ServiceComponent.ARCHIVE_S1SVC.getComponent() +" | ID-5005 | COPY-DIR-ERROR | S3FileUtil | " + sourceS3Dir.getKey() + " | "
          + "directory not found"  + " ]");
    }
    
    removeFile(sourceS3Dir.getKey() + EXCLUDED_FILE, sourceS3Dir.getBucketName());
    ...
    if (stat.getNumOfFilesTotal().intValue() == 0 || stat.getNumOfFilesTotal().intValue() != stat.getNumOfFilesSuccess().intValue())
    {
    ...
    if (files != null)
    {
      for (int i = 0; i < files.length; i++)
      {
        ...
        try
        {
          ...
          log.info("[ " + ServiceComponent.ARCHIVE_S1SVC.getComponent() +" | ID-5013 | DELETE-FILE-SUCCESS | S3FileUtil | " + "(" + file.getBucketName() + ":" + file.getKey() +  " : " + file.getVersionId() + ")" + " ]");
        }
        catch(AmazonServiceException amazonServiceException)
        {
          log.error("[ " + ServiceComponent.ARCHIVE_S1SVC.getComponent() +" | ID-5013 | DELETE-FILE-ERROR | S3FileUtil | " + "(" + file.getBucketName() + ":" + file.getKey() +  " : " + file.getVersionId() + ")" + " | "
              + amazonServiceException.getMessage() + " ]");
        }
  ...
  public static int createExclusionsFile(S3File s3Dir, List<String> exclusions, String archiveCacheDir)
  {
    ...
    try
    {
      ...
      log.info("[ " + ServiceComponent.ARCHIVE_S1SVC.getComponent() +" | ID-5007 | CREATE-EXCLUSION-SUCCESS | S3FileUtil | "
          + excludeFile + " | " + s3Dir.getKey() + "  ]");
    }
    catch(Exception e)
    {
      log.error("[ " + ServiceComponent.ARCHIVE_S1SVC.getComponent() + " | ID-5007 | CREATE-EXCLUSION-FILE-ERROR | S3FileUtil | "
  ...
  public static boolean isDirectory(String key, String bucketName)
  {
    ...
    {
      log.error(
          "[ " + ServiceComponent.ARCHIVE_S1SVC.getComponent() + " | ID-5008 | IS-DIRECTORY-ERROR | S3FileUtil | "
  ...
  public static boolean exists(String key , String bucketName)
  {
    ...
    {
      log.error("[ " + ServiceComponent.ARCHIVE_S1SVC.getComponent() +" | ID-5009 | EXISTS-ERROR | S3FileUtil | " + "(" + bucketName + ":" + key + ")" + " | "
          + amazonServiceException.getMessage() + " ]");
    }
    catch(SdkClientException sce)
    {
  ...
  public static S3File[] listFiles(S3File file)
  {
    ...
    {
      log.error("[ " + ServiceComponent.ARCHIVE_S1SVC.getComponent() +" | ID-5010 | LIST-FILES-ERROR | S3FileUtil | " + "(" + file.getBucketName() + ":" + file.getKey() + ")" + " | "
          + amazonServiceException.getMessage() + " ]");
      return null;
    }
    catch(SdkClientException sce)
  ...
  public static String[] listFileKeys(S3File file)
  {
    ...
    {
      log.error("[ " + ServiceComponent.ARCHIVE_S1SVC.getComponent() +" | ID-5011 | LIST-FILE-KEYS-ERROR | S3FileUtil | " + "(" + file.getBucketName() + ":" + file.getKey() + ")" + " | "
          + e.getMessage() + " ]");
      return null;
    }
    catch (SdkClientException sce)
  ...
  public static void shutdownS3Daemons()
  {
    ...
    {
      log.error("[ " + ServiceComponent.ARCHIVE_S1SVC.getComponent() +" | ID-5012 | SHUTDOWN-S3-DAEMONS | S3FileUtil | " + " | "
          + e.getMessage() + " ]");
    }
  }
